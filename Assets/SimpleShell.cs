...
public class SimpleShell : MonoBehaviour {
    ...
    void Update() {
        ...
        //set wind direction from angle
        direction.x = Mathf.Cos(windDirection * Mathf.Deg2Rad);
        direction.z = Mathf.Sin(windDirection * Mathf.Deg2Rad);
        ...
        //set direction as a global variable
        ...
        //always update displacement strength so wind works
        for (int i = 0; i < shellCount; ++i) {
            shells[i].GetComponent<MeshRenderer>().material.SetFloat("_DisplacementStrength", displacementStrength*(Mathf.Sin(Time.time%6.28f)+1f));
        }    
    }
